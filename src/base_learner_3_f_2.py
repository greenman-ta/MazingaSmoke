...
from utils_2 import build_shadow_mix, TemporalFlicker 
...
class BaseLearner(ABC):
    ...
    def load(self, model, in_path, rank=0, ignore_fc=False, fill_dim=False):
        if model is not None and in_path is not None:
            ...
            # Map to CUDA only if CUDA is available; otherwise load on CPU (works on CPU and MPS too)
            if torch.cuda.is_available():
                map_loc = lambda storage, loc: storage.cuda(rank)
            else:
                map_loc = 'cpu'
            sd_loaded = torch.load(in_path, map_location=map_loc)
    ...
    def get_transform(self, mode, phase=None, image_size=224, ep=0, seed=0):
        if mode == "rgb": # three channels (r, g, and b)
            mean = (0.485, 0.456, 0.406) #mod ghostnetv2
        ...
        if phase == "train":
            ...
            if mode == "rgb" or mode == "rgbd":
                ...
                if ep >= 14:
                    shadow_mix = build_shadow_mix(
                        base_seed=int(seed),
                        epoch=int(ep),
                        start_epoch=1,   # o quando vuoi attivarlo
                        p=0.25           # se vuoi 0 per disattivarlo
                    )
                    '''
                    tf = TemporalFlicker(
                            p=0.20,
                            gain_delta=0.10,
                            gamma_delta=0.00,
                            base_seed=int(seed),   # <--- IMPORTANTISSIMO
                            epoch=int(ep),         # <--- IMPORTANTISSIMO
                            start_epoch=14
                        )'''
                    return Compose([cj,shadow_mix, rrc, rp, rhf, tt, nm, re, re])
                else:
                    return Compose([cj, rrc, rp, rhf, tt, nm, re, re])
                ...
                return Compose([rrc, rp, rhf, tt, nm])
        else:
