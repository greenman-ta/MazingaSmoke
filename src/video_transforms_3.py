        ...
        for attempt in range(10):
            ...
            if h < img_h and w < img_w:
                ...
                if isinstance(value, numbers.Number):
                    ...
                    # string value (e.g. 'random') -> fill with random noise
...
class ToTensor(object):
    ...
    def __call__(self, imgs):
        ...
        for I in imgs:
            t = F.to_tensor(I)
            # Always ensure float32 in the pipeline (don't rely on F.to_tensor behavior)
            t = t.to(dtype=torch.float32)
            # If values are in 0..255 (common when upstream kept uint8 range), scale to 0..1
            if t.max() > 1.0:
                t = t.div(255.0)
            output_imgs.append(t.unsqueeze(1))
        return torch.cat(output_imgs, dim=1)

    def __repr__(self):
        return self.__class__.__name__ + '()'

class ReflectPad(object):
    def __init__(self, pad=8): self.pad=int(pad)
    def __call__(self, imgs):
        T,H,W,C = imgs.shape
        out=[]
        for I in imgs:
            J = cv2.copyMakeBorder(I, self.pad, self.pad, self.pad, self.pad,
...
class RandomPerspectiveReflect(object):
    ...
    def __call__(self, imgs):
        ...
        offset_h = int(float(half_h) * self.distortion_scale)
        offset_w = int(float(half_w) * self.distortion_scale)
        
        tl = [random.randint(0, offset_w), random.randint(0, offset_h)]
        tr = [random.randint(W - 1 - offset_w, W - 1), random.randint(0, offset_h)]
        bl = [random.randint(0, offset_w), random.randint(H - 1 - offset_h, H - 1)]
        br = [random.randint(W - 1 - offset_w, W - 1), random.randint(H - 1 - offset_h, H - 1)]
