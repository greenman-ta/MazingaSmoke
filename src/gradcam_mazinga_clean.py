...
def vis_saliency_5rows_singleclip(
    ...
    topk_idx: List[int],
    post_attn_maps_k: Optional[Union[np.ndarray, torch.Tensor]] = None,  # [K,7,7] or [K,1,7,7]
    ...
    for t in frame_idx:
        ...
        cams_back[t] = cam_single(F_hook[bt], G_hook[bt])   # F_hook: [B*T,C,Hf,Wf]

    # POST-FUSION BACKWARD CAM (da fusion_module.last_feat + grad)
    fm = model.fusion_module.last_feat          # [B,K,C,Hf,Wf]
    gm = fm.grad                                 # stesso shape
    F_fus = fm[0]
    G_fus = gm[0]
    ...
    # STATIC BRANCH CAM (da static_branch.last_feat + grad)
    ...
    for ci, t in enumerate(frame_idx):
        ...
        ax.set_title("Teacher attn_mapK")
        ax.axis("off")
