...
def vis_saliency_5rows_singleclip(
    ...
    if w_eff is not None:
        ...
        eff_np = np.asarray(eff_np, dtype=np.float32).reshape(-1)

        n = min(len(idx_np), len(eff_np))
        idx_np = idx_np[:n]
        eff_np = eff_np[:n]

        sel = np.argsort(-eff_np)[:max_frames]   
        frame_idx = idx_np[sel].tolist()

        # riordino temporale per la visualizzazione
        frame_idx = sorted(frame_idx)
    else:
        frame_idx = idx_np[:max_frames].tolist()
