...
def linear_ramp(ep: int, start: int, end: int, v0: float, v1: float):
    if ep <= start:
        return float(v0)
    if ep >= end:
        return float(v1)
...
def focal_gamma_schedule(
    ep: int,
    warm_end: int = 4,        # epoche di warmup
    ramp_end: int = 12,       # fine ramp verso gamma finale
    g0: float = 0.0,          # gamma iniziale
...
def set_loss_gamma(loss_obj, gamma: float):
...
BOOT_CFG = {
    ...
    "start_epoch": 14,     # da quando parte
    "disable_on_mixup": True,  
}
