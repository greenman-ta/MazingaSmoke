...
from .ca_fusion_multi import CAFusion 
...
class MazingaSmokeClassifier(nn.Module):
    ...
    def _build_static_in(self, f_deep, f_int, topK):

        B, T, Cd, Hd, Wd = f_deep.shape
        _, _, Ce, He, We = f_int.shape
        K = topK.shape[1]

        # gather deep/int su topK
        idx_deep = topK[..., None, None, None].expand(B, K, Cd, Hd, Wd)
        idx_int  = topK[..., None, None, None].expand(B, K, Ce, He, We)

        f_k     = f_deep.gather(1, idx_deep)   # [B,K,Cd,Hd,Wd]
        f_int_k = f_int.gather(1, idx_int)     # [B,K,Ce,He,We]

        topK_prev = (topK - 1).clamp(min=0)
        idx_prev  = topK_prev[..., None, None, None].expand(B, K, Ce, He, We)
        f_int_prev = f_int.gather(1, idx_prev) # [B,K,Ce,He,We]
    ...
    def forward(self, x, d_mod=128, ft=False):
        ...
        # --- SELECTA TOP-K FRAMES ---
        topK, w_topk = self.selecta(f_deep, w) #ottengo indici topK e pesi corrispondenti

        # costruisco input static branch
        static_in = self._build_static_in(f_deep, f_int, topK)

        # --- STATIC BRANCH ---
        f_static = self.static_branch(static_in)   # [B,K,128,Hd,Wd]
        
        # --- GATHER feature temporali corrispondenti ai top-K ---
        t_K = x_seq.gather(
            1,
            topK.unsqueeze(-1).expand(-1, -1, x_seq.size(-1))
        )  # [B, K, d_mod]

        #fusione
        f_k, attn_mapK, f_fused = self.fusion_module(f_static, t_K, topk_idx=topK, w_topk=w_topk)
        ...
        try:
            K = topK.shape[1]
            self.last_vis = {
                "topk_idx": topK[0].detach().cpu(),                 
                "topk_order": torch.arange(K).detach().cpu(),      
                "post_attn_maps_k": attn_mapK[0].detach().cpu(),    
                "w_topk": w_topk[0].detach().cpu() if isinstance(w_topk, torch.Tensor) else None,
            }
        except Exception:
            self.last_vis = {}
